...
import { MoreHorizontal, Edit, Trash, CheckCircle, Eye, AlertCircle } from "lucide-react"
          ...
          <TableBody>
            {filteredSupplies.map((supply, index) => (
              <TableRow key={supply.id}>
                ...
                <TableCell>{supply.expectedQuantity}</TableCell>
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end">
                      ...
                      {session?.user.role === "commander" && (
                        <DropdownMenuItem onClick={() => handleView(supply)}>
                      ...
                      {session?.user.role === "brigadeAssistant" && supply.status === "pending" && (
                        <DropdownMenuItem onClick={() => handleApprove(supply)}>
                      ...
                      {session?.user.role === "brigadeAssistant" && supply.status === "approved" && (
                        <DropdownMenuItem onClick={() => handleView(supply)}>
                          <Eye className="mr-2 h-4 w-4" />
                          Xem chi tiết
                        </DropdownMenuItem>
                      ...
                      {session?.user.role === "unitAssistant" &&
                        supply.status === "pending" &&
                        supply.unit._id === session.user.unit && (
                          <>
                            <DropdownMenuItem onClick={() => handleEdit(supply)}>
                              <Edit className="mr-2 h-4 w-4" />
                              Chỉnh sửa
                            </DropdownMenuItem>
                            <DropdownMenuItem onClick={() => handleDelete(supply)}>

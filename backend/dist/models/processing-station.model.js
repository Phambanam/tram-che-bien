...
exports.PoultryProcessing = exports.LivestockProcessing = exports.SausageProcessing = exports.BeanSproutsProcessing = exports.SaltProcessing = exports.TofuProcessing = void 0;
const mongoose_1 = __importDefault(require("mongoose"));
// Tofu Processing
const tofuProcessingSchema = new mongoose_1.default.Schema({
    ...
    supervisedBy: { type: mongoose_1.default.Schema.Types.ObjectId, ref: 'User' },
    notes: { type: String }
}, { timestamps: true });
// Salt Processing  
const saltProcessingSchema = new mongoose_1.default.Schema({
    date: { type: Date, required: true },
    unitId: { type: mongoose_1.default.Schema.Types.ObjectId, ref: 'Unit', required: true },
    input: {
        cabbage: {
...
const beanSproutsProcessingSchema = new mongoose_1.default.Schema({
    date: { type: Date, required: true },
    unitId: { type: mongoose_1.default.Schema.Types.ObjectId, ref: 'Unit', required: true },
    input: {
        soybeans: {
...
// Sausage Processing
const sausageProcessingSchema = new mongoose_1.default.Schema({
    date: { type: Date, required: true },
    unitId: { type: mongoose_1.default.Schema.Types.ObjectId, ref: 'Unit', required: true },
    input: {
        porkMeat: {
...
const livestockProcessingSchema = new mongoose_1.default.Schema({
    date: { type: Date, required: true },
    unitId: { type: mongoose_1.default.Schema.Types.ObjectId, ref: 'Unit', required: true },
    input: {
        livePigs: {
...
// Poultry Processing
const poultryProcessingSchema = new mongoose_1.default.Schema({
    date: { type: Date, required: true },
    unitId: { type: mongoose_1.default.Schema.Types.ObjectId, ref: 'Unit', required: true },
    input: {
        livePoultry: {
...
schemas.forEach(({ schema }) => {
    ...
    schema.methods.calculateOutputValue = function () {
        let total = 0;
        if (this.output.tofu)
            total += this.output.tofu.quantity * this.output.tofu.pricePerKg;
        if (this.output.pickledCabbage)
            total += this.output.pickledCabbage.quantity * this.output.pickledCabbage.pricePerKg;
        if (this.output.beanSprouts)
            total += this.output.beanSprouts.quantity * this.output.beanSprouts.pricePerKg;
        if (this.output.sausages)
            total += this.output.sausages.quantity * this.output.sausages.pricePerKg;
        if (this.output.leanMeat)
            total += this.output.leanMeat.quantity * this.output.leanMeat.pricePerKg;
        if (this.output.bones)
            total += this.output.bones.quantity * this.output.bones.pricePerKg;
        if (this.output.groundMeat)
            total += this.output.groundMeat.quantity * this.output.groundMeat.pricePerKg;
        if (this.output.organs)
            total += this.output.organs.quantity * this.output.organs.pricePerKg;
        if (this.output.processedMeat)
            total += this.output.processedMeat.quantity * this.output.processedMeat.pricePerKg;
        return total;
    };
    schema.methods.calculateInputCost = function () {
        let total = 0;
        if (this.input.soybeans)
            total += this.input.soybeans.quantity * this.input.soybeans.price;
        if (this.input.cabbage)
            total += this.input.cabbage.quantity * this.input.cabbage.price;
        if (this.input.porkMeat)
            total += this.input.porkMeat.quantity * this.input.porkMeat.price;
        if (this.input.livePigs)
            total += this.input.livePigs.quantity * this.input.livePigs.price;
        if (this.input.livePoultry)
            total += this.input.livePoultry.quantity * this.input.livePoultry.price;
        return total;
    };
});
// Export models
exports.TofuProcessing = mongoose_1.default.model('TofuProcessing', tofuProcessingSchema);
exports.SaltProcessing = mongoose_1.default.model('SaltProcessing', saltProcessingSchema);
exports.BeanSproutsProcessing = mongoose_1.default.model('BeanSproutsProcessing', beanSproutsProcessingSchema);
exports.SausageProcessing = mongoose_1.default.model('SausageProcessing', sausageProcessingSchema);
exports.LivestockProcessing = mongoose_1.default.model('LivestockProcessing', livestockProcessingSchema);
exports.PoultryProcessing = mongoose_1.default.model('PoultryProcessing', poultryProcessingSchema);

...
exports.getDailyIngredientSummaries = exports.getMenuPlanningOverview = exports.generateDailyMenuPlan = exports.getInventoryAlerts = exports.getMenuSuggestions = void 0;
...
const getMenuSuggestions = async (req, res) => {
    try {
        ...
        for (const dish of dishes) {
            ...
            let priority = "low";
            const reasons = [];
            const ingredientAnalysis = dish.ingredients.map((ingredient) => {
                const inventoryItem = inventory.find((inv) => inv.product._id.toString() === ingredient.lttpId);
            ...
            const insufficientIngredients = ingredientAnalysis.filter((ing) => ing.status === "insufficient" || ing.status === "expired");
            const expiringSoon = ingredientAnalysis.filter((ing) => ing.status === "expiring_soon");
...
// @access  Private
const generateDailyMenuPlan = async (req, res) => {
    try {
        const { date } = req.body;
        if (!date) {
        ...
        const highPriority = suggestions.filter((s) => s.priority === "high").slice(0, 2);
        const mediumPriority = suggestions.filter((s) => s.priority === "medium").slice(0, 3);
        const lowPriority = suggestions.filter((s) => s.priority === "low").slice(0, 3);
...
exports.generateDailyMenuPlan = generateDailyMenuPlan;
// @desc    Get comprehensive menu planning data
// @route   GET /api/menu-planning/overview
// @access  Private
const getMenuPlanningOverview = async (req, res) => {
    try {
        const db = await (0, database_1.getDb)();
        if (!db) {
            return res.status(500).json({
        ...
        const criticalAlerts = alertsResponse.data.filter((alert) => alert.alertLevel === "critical").length;
        res.status(200).json({
            success: true,
            data: {
                totalPersonnel,
...
exports.getMenuPlanningOverview = getMenuPlanningOverview;
// @desc    Get daily ingredient summaries for menu planning
// @route   GET /api/menu-planning/ingredient-summaries
// @access  Private
const getDailyIngredientSummaries = async (req, res) => {
    try {
...
exports.getDailyIngredientSummaries = getDailyIngredientSummaries;

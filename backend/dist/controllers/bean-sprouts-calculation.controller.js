...
exports.beanSproutsCalculationService = exports.getMonthlyBeanSproutsSummary = exports.getDailyBeanSproutsProcessing = exports.updateDailyBeanSproutsProcessing = exports.getWeeklyBeanSproutsTracking = exports.getBeanSproutsUsageStatistics = exports.calculateWeeklyBeanSproutsRequirements = exports.calculateBeanSproutsRequirements = void 0;
...
const getBeanSproutsUsageStatistics = async (req, res) => {
    try {
        ...
        const beanSproutsProcessingData = await db.collection("dailyTofuProcessing")
            ...
            .sort({ date: 1 })
            .toArray();
        const totalProcessedBeanSprouts = beanSproutsProcessingData.reduce((sum, data) => sum + (data.tofuInput || 0), 0 // Map from tofu field names
        );
        const totalSoybeansUsed = beanSproutsProcessingData.reduce((sum, data) => sum + (data.soybeanInput || 0), 0 // Map from tofu field names
        );
...
// @access  Private
const updateDailyBeanSproutsProcessing = async (req, res) => {
    try {
        const { date, soybeansInput, beanSproutsInput, beanSproutsOutput, soybeansPrice, beanSproutsPrice, byProductQuantity, byProductPrice, otherCosts, note } = req.body;
        if (!date) {
            return res.status(400).json({
                success: false,
        ...
        // Upsert the data
        await db.collection("dailyBeanSproutsProcessing").updateOne({ date }, { $set: processingData }, { upsert: true });
        res.status(200).json({
            success: true,
...
exports.updateDailyBeanSproutsProcessing = updateDailyBeanSproutsProcessing;
// @desc    Get daily bean sprouts processing data
// @route   GET /api/bean-sprouts-calculation/daily-processing
// @access  Private
const getDailyBeanSproutsProcessing = async (req, res) => {
    try {
        ...
        const db = await (0, database_1.getDb)();
        const processingData = await getBeanSproutsProcessingStationData(db, date);
        res.status(200).json({
            success: true,
            data: processingData
        });
...
exports.getDailyBeanSproutsProcessing = getDailyBeanSproutsProcessing;
...
async function getMonthlyBeanSproutsProcessingDataWithFinancials(db, year, month) {
    try {

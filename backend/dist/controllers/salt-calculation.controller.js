...
exports.saltCalculationService = exports.getMonthlySaltSummary = exports.getWeeklySaltTracking = exports.getSaltUsageStatistics = exports.calculateWeeklySaltRequirements = exports.calculateSaltRequirements = void 0;
const mongodb_1 = require("mongodb");
const database_1 = require("../config/database");
// @desc    Calculate salt requirements from menu and unit personnel
// @route   GET /api/salt-calculation/requirements
// @access  Private
const calculateSaltRequirements = async (req, res) => {
    try {
                ...
                if (meal.dishDetails && Array.isArray(meal.dishDetails)) {
                    for (const dish of meal.dishDetails) {
                        if (dish.ingredients && Array.isArray(dish.ingredients)) {
                            const saltIngredients = dish.ingredients.filter((ing) => ing.lttpName.toLowerCase().includes('dưa muối') ||
                            ...
                            if (saltIngredients.length > 0) {
                                ...
                                const dishInfo = {
                                    dishName: dish.name,
                                    mealType: meal.type,
                                    saltIngredients: saltIngredients.map((ing) => ({
                                        lttpId: ing.lttpId,
                                ...
                                const existingDish = result.dishesUsingSalt.find(d => d.dishName === dish.name && d.mealType === meal.type);
                                if (!existingDish) {
...
exports.calculateSaltRequirements = calculateSaltRequirements;
...
// @access  Private
const calculateWeeklySaltRequirements = async (req, res) => {
    try {
...
exports.calculateWeeklySaltRequirements = calculateWeeklySaltRequirements;
// Helper function to calculate salt for a specific date
async function calculateDailySaltForDate(db, targetDate, unitIds) {
    // Find menu containing this date
    const selectedDate = new Date(targetDate);
    const menu = await db.collection("menus").findOne({
        startDate: { $lte: selectedDate },
...
// @access  Private
const getSaltUsageStatistics = async (req, res) => {
    try {
        ...
        const saltProcessingData = await db.collection("dailySaltProcessing")
            ...
            .sort({ date: 1 })
            .toArray();
        const totalProcessedSalt = saltProcessingData.reduce((sum, data) => sum + (data.saltInput || 0), 0);
        const totalCabbageUsed = saltProcessingData.reduce((sum, data) => sum + (data.cabbageInput || 0), 0);
        ...
        res.status(200).json({
            ...
            data: {
                ...
                saltDishes: saltDishes.map(dish => ({
                    ...
                    saltIngredients: dish.ingredients.filter((ing) => ing.lttpName.toLowerCase().includes('dưa muối') ||
                        ing.lttpName.toLowerCase().includes('muối nén') ||
                        ing.lttpName.toLowerCase().includes('dưa chua') ||
                        ing.lttpName.toLowerCase().includes('dưa cải'))
                })),
...
exports.getSaltUsageStatistics = getSaltUsageStatistics;
...
// @access  Private
const getWeeklySaltTracking = async (req, res) => {
    try {
...
exports.getMonthlySaltSummary = getMonthlySaltSummary;
// Helper functions
...
exports.saltCalculationService = {
